...
from frappe.utils import date_diff, flt, getdate, logger

logger.set_log_level("DEBUG")
logger = frappe.logger("po bepl", allow_site=True, file_count=1)
...
def get_data(filters):
	...
	query = (
		...
		.where(
			(po_item.parent == po.name) & (po.status.notin(("Stopped", "Closed")))
		)
	...
	if filters.get("status"):
		...
		is_draft = "Draft" in status
		if is_draft:
			query = query.where(po.docstatus == 0)
